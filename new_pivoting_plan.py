...
if __name__=="__main__":
    ...
    goal_no = 3
    OBJ = get_OBJ(object_type,goal_no)
    # plot_3d_object(OBJ)
    OBJ_neighbors = get_neighbors(OBJ)
    plot_3d_object(OBJ)
    plot_3d_object(unfold_object(1,OBJ,OBJ_neighbors))
    ...
    data_name = ["Object type","Goal no","d_l","normal_l","distal_l","d_r","normal_r","distal_r","z","time"]
    param_file.write(str(data_name) + "\n")
    plan_file = open(file_path+"/data.txt","w")
    data_name = ["Step","command"]
    plan_file.write(str(data_name)+"\n")

    data = [object_type,goal_no,d_l,normal_l,distal_l,d_r,normal_r,distal_r,z,dt]
